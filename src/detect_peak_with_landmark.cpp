...
void Modify_cloud(CloudAPtr shape_cloud)
{
	size_t cloud_size = shape_cloud->points.size();
	cout<<"modify cloud_size"<<cloud_size<<endl;
	for(size_t i=door_degree-10;i<door_degree+10;i++){
		shape_cloud->points[i].intensity = 40.0;
	}
}
...
{
	door_degree = (180 - (int)(msg.data*180.0/M_PI))*2;
	cout<<"door_degree"<<door_degree<<endl;
