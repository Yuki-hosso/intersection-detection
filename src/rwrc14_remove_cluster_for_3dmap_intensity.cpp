...
sensor_msgs::PointCloud2 removed_pt_map;
...
{	
	...
	if(MODE==1)original_sub = n.subscribe("/intensity/height", 1, originalCallback);
	// ros::Subscriber centroid_sub = n.subscribe("/human_recognition/positive_position", 1, objectCallback);
	ros::Subscriber centroid_sub = n.subscribe("/cluster/like_human", 1, objectCallback);
	...
	ros::Publisher removed_pt_pub = n.advertise<sensor_msgs::PointCloud2>("/intensity/remove",1);
	ros::Publisher removed_map_pub = n.advertise<sensor_msgs::PointCloud2>("/local_cloud/map",1);
	...
	while (ros::ok()){
		...
		if(callback_flag){
			...
			pcl::toROSMsg(removed_pcl,removed_pt_map);
			removed_pcl.points.clear();
			removed_pt_map.header.frame_id="/matching_base_link";
			// removed_pt_map.header.frame_id="/map";
			removed_pt_map.header.stamp=ros::Time::now();
			removed_map_pub.publish(removed_pt_map);
