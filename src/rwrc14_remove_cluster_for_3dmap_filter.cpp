...
sensor_msgs::PointCloud2 filtered_pt;
	...
	for(size_t i=0;i<pcl_in.points.size();i++){
		...
		if(THRESH_DIST>dead_line){
			...
			for(size_t j=0;j<centroid.points.size();j++){
				...
				if((centroid.points[min_id].normal_x/1.0 + 0.7)>min_dist){
					break;
				}
			}
			// if((centroid.points[min_id].normal_x/1.0)>min_dist){
...
{	
	...
	if(MODE==1)original_sub = n.subscribe("/static_cloud", 1, originalCallback);
	ros::Subscriber centroid_sub = n.subscribe("/human_recognition/positive_position", 1, objectCallback);
	// ros::Subscriber centroid_sub = n.subscribe("/cluster/select/centroid", 1, objectCallback);
	////publisher////
	...
	ros::Publisher removed_pt_pub = n.advertise<sensor_msgs::PointCloud2>("/local_cloud",1);
	ros::Publisher removed_filter_pub = n.advertise<sensor_msgs::PointCloud2>("/local_filtered",1);
	...
	while (ros::ok()){
		...
		if(callback_flag){
			...
			pcl::PointCloud<pcl::PointXYZINormal>::Ptr removed_pcl_ptr(new pcl::PointCloud<pcl::PointXYZINormal>);
			pcl::PointCloud<pcl::PointXYZINormal>::Ptr filtered_pcl(new pcl::PointCloud<pcl::PointXYZINormal>);
			...
			size_t point_size = removed_pcl.points.size();
			for(size_t i = 0;i<point_size;i++){
				removed_pcl_ptr->points.push_back(removed_pcl.points[i]);
			}
 			// Create the filtering object
 			pcl::StatisticalOutlierRemoval<pcl::PointXYZINormal> sor;
 			sor.setInputCloud (removed_pcl_ptr);
 			sor.setMeanK (10);
 			sor.setStddevMulThresh (10.0);
 			sor.filter (*filtered_pcl);

			pcl::toROSMsg(*filtered_pcl,filtered_pt);
			removed_pcl.points.clear();
			filtered_pcl->points.clear();
			filtered_pt.header.frame_id="/velodyne";
			filtered_pt.header.stamp=ros::Time::now();
			removed_filter_pub.publish(filtered_pt);
