...
void Calc_shape(CloudAPtr input_cloud)
{
	...
	for(size_t i=0;i<cloud_size;i++){
		...
		input_cloud->points[i].intensity = atan2(input_cloud->points[i].y,input_cloud->points[i].x)/PI*360.0;
...
int main (int argc, char** argv)
{
    ...
    shape_pub = nh.advertise<sensor_msgs::PointCloud2> ("/detect_shape", 1);
    // main handle
    ros::Rate loop_rate(20);
    while (ros::ok()){
		if(callback_flag){
			Calc_shape(tmp_cloud);
			callback_flag = false;
		}
        ros::spinOnce();
        loop_rate.sleep();
